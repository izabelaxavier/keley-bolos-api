    ...
    public byte[] gerarComprovante(Pedido pedido) {
        ...
        try {
            ...
            for (ItemPedido item : pedido.getItens()) {
                String linha = String.format("%dx %s - %s",
                        item.getQuantidade(),
                        item.getProduto().getNome(), // O nome ja vem sem acento do banco
            ...
            if (pedido.getTaxaMaquininha() != null && pedido.getTaxaMaquininha().doubleValue() > 0) {
                document.add(new Paragraph("Taxa Maquininha: " + nf.format(pedido.getTaxaMaquininha())));
            }

            Font fonteTotal = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 14);
            Paragraph total = new Paragraph("VALOR TOTAL: " + nf.format(pedido.getValorTotal()), fonteTotal);
